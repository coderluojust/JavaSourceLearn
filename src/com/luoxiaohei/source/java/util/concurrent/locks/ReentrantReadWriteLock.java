...
public class ReentrantReadWriteLock
    ...
    abstract static class Sync extends AbstractQueuedSynchronizer {
        ...
        protected final boolean tryRelease(int releases) {
            ...
            // 如果state中低16位=0则表明写锁被释放完全，清理持有线程
        ...
        protected final boolean tryAcquire(int acquires) {
            ...
            if (c != 0) {
                ...
                // 当 state 不为0，而写状态等于0，则读状态大于0，说明读锁已被获取
                // 读锁已被获取，此时除非是重入，否则不能获取写锁，直接返回fale，获取写锁失败
                if (w == 0 || current != getExclusiveOwnerThread())
                    return false;
                // 写锁拥有的是state的低16位，这里需要校验是否大于其16位能表示的数据范围
                if (w + exclusiveCount(acquires) > MAX_COUNT)
                    throw new Error("Maximum lock count exceeded");
                // Reentrant acquire
                // 思考下：能到这里，说明写锁状态不为0，且当前线程为已经获取写锁的线程，这块反映了支持可重入
                setState(c + acquires);
                return true;
            }
        ...
        protected final boolean tryReleaseShared(int unused) {
            ...
            if (firstReader == current) {
                ...
                int count = rh.count;
                if (count <= 1) {
                    // 这一步将 ThreadLocal remove 掉，防止内存泄漏。因为当前线程已经不再持有读锁了
                    readHolds.remove();
                    if (count <= 0)
        ...
        protected final int tryAcquireShared(int unused) {
            ...
            Thread current = Thread.currentThread();
            int c = getState();
            // 如果写锁已经被持有，且持有写锁的不是当前线程，直接失败
            if (exclusiveCount(c) != 0 &&
                getExclusiveOwnerThread() != current)
                return -1;
            // c >>> 16 获取state中表示读锁的数值
            int r = sharedCount(c);
            // 读锁获取是否需要被阻塞
            if (!readerShouldBlock() &&
                // 判断是否会溢出,毕竟只占16位 (2^16-1，没那么容易溢出的)
                r < MAX_COUNT &&
                ...
                if (r == 0) {
                    ...
                    // 如果获取读锁的线程是第一个获取读锁的，则代表重入，其持有读锁数量自增1
                    firstReaderHoldCount++;
                } else {
         ...
         */
        /**
         * 1. 上面看到由于CAS失败，本来要进入阻塞队列，但是不甘心呀，好不容易满足了 ！readerShouldBlock() ，所以这里会增加CAS成功的几率
         * 2. 在上面的 tryAcquireShared 中没有处理可重入读取，所以如果是读锁，虽然它排在 head.next 写锁之后，但是可重入读就不管你这个了
         * @param current
         * @return
         */
